...
export default function VsAI() {
    ...
    const AIMove = (newBoard: BoardState | null) => {
        ...
        if (checkPut(AIBoard, AI)) {
            ...
            thinkingProcess += `AIの置ける場所: ${canPutAI.join(', ')}\n`;
            thinkingProcess += "各手の相手の選択肢数:\n";

            canPutAI.forEach((cell, index) => {
                thinkingProcess += `位置 ${cell}: ${canPutPlayerPerAI[index].length}手\n`;
            });
            ...
            if (countPieces.countBlack + countPieces.countWhite <= 48) {
                ...
                thinkingProcess += "最も多くの石を取れる手を選びます。\n";
                makeBoardAfterAI(maxAIPut, AIBoard);
            }
        } else {
            thinkingProcess += "AIはパスします。\n";
            setTimeout(() => {
        ...
        setAiThinkingProcess(thinkingProcess);
    }
        ...
        <div className='flex flex-col items-center'>
            ...
            {aiThinkingProcess !== "" && (
                <div className="mt-4 p-4 bg-gray-100 rounded">
                    <h3 className="font-bold">AIの思考過程:</h3>
                    <pre className="whitespace-pre-wrap">{aiThinkingProcess}</pre>
                </div>
            )}
        </div>
    );
